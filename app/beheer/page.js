  ...
  return (
    <div className="page" style={{ maxWidth: 820 }}>
                ...
                <>
                  <div className="card-row">
                    ...
                    <div style={{ textAlign: "right" }}>
                      <span className={`tag ${session.is_full ? "full" : "open"}`}>
                        {session.participant_count} aangemeld{session.is_full ? " · vol" : ""}
                      </span>
                      {session.participant_count < session.min_participants && (
                        <div className="muted" style={{ marginTop: "0.3rem" }}>
                          Nog onder minimum ({session.min_participants})
                        </div>
                      )}
                      <div style={{ marginTop: "0.6rem", display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                        <button className="btn" onClick={() => setEditingSessionId(session.id)}>Bewerken</button>
                        <button className="btn" onClick={() => deleteSession(session.id)}>Verwijderen</button>
                      </div>
                    </div>
                  </div>
                  {session.bookings && session.bookings.length > 0 && (
                    <table style={{ marginTop: "1rem" }}>
                      <thead>
                        <tr><th>Naam</th><th>E-mail</th><th>Telefoon</th></tr>
                      </thead>
                      <tbody>
      ...
      <section style={{ marginTop: "3rem" }}>
        <div className="card-row">
          <h2>1-op-1 beschikbaarheid</h2>
          {!addingSlot && (
            <button className="btn" onClick={() => setAddingSlot(true)}>+ Nieuw tijdslot</button>
          )}
        </div>
        ...
        {addingSlot && (
          <div className="card">
            <SlotForm
              initial={{ date: "", start_time: "", end_time: "" }}
              saving={saving}
        ...
        {slots === null && <p className="muted">Bezig met laden…</p>}
        {slots &&
          slots.map((slot) => (
            <div className="card" key={slot.id}>
              {editingSlotId === slot.id ? (
                <SlotForm
                  ...
                  onSave={(form) => updateSlot(slot.id, form)}
                />
              ) : (
                <div className="card-row">
                  <div>
                    ...
                    {slot.booking && (
                      <div className="muted" style={{ marginTop: "0.3rem" }}>
                        Geboekt door {slot.booking.name} ({slot.booking.email}, {slot.booking.phone})
                      </div>
                    )}
                  </div>
                  <div style={{ textAlign: "right" }}>
                    <span className={`tag ${slot.is_booked ? "full" : "open"}`}>
                      {slot.is_booked ? "Geboekt" : "Vrij"}
                    </span>
                    <div style={{ marginTop: "0.6rem", display: "flex", gap: "0.5rem", justifyContent: "flex-end" }}>
                      <button className="btn" onClick={() => setEditingSlotId(slot.id)}>Bewerken</button>
                      <button className="btn" onClick={() => deleteSlot(slot.id)}>Verwijderen</button>
                    </div>
                  </div>
                </div>
              )}
...
export default function BeheerPage() {
  ...
  useEffect(() => {
    // We checken de sessie door een admin-only call te doen en te kijken
    // of we de admin-only data (bookings) terugkrijgen.
    fetch("/api/sessions")
      .then((r) => r.json())
      .then((data) => {
        const hasAdminData = (data.sessions || []).some((s) => s.bookings !== undefined);
        setStatus(hasAdminData ? "loggedIn" : "loggedOut");
      })
      .catch(() => setStatus("loggedOut"));
  }, []);

  if (status === "checking") return null;
  if (status === "loggedOut") return <LoginForm onLoggedIn={() => setStatus("loggedIn")} />;
  return <Dashboard onLoggedOut={() => setStatus("loggedOut")} />;
}
