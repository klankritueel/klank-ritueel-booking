    ...
    <div className="page">
                  ...
                  <div style={{ textAlign: "right" }}>
                    ...
                    <div style={{ marginTop: "0.6rem" }}>
                      <button
                        ...
                        onClick={() =>
                          setOpenSessionForm(openSessionForm === session.id ? null : session.id)
                        }
                      >
                        {session.is_full ? "Vol" : "Boek je plek"}
                      </button>
                    </div>
                  </div>
                </div>
                {openSessionForm === session.id && !session.is_full && (
                  <div style={{ marginTop: "1.25rem", borderTop: "1px solid var(--border)", paddingTop: "1.25rem" }}>
                    <BookingForm
                      submitLabel="Bevestig aanmelding"
                      submitting={state.submitting}
                      error={state.error}
                      success={state.success}
                      onSubmit={(form) => submitSessionBooking(session, form)}
                    />
                  </div>
                )}
              </div>
      ...
      <section style={{ marginTop: "3rem" }}>
        ...
        {slots && slots.length === 0 && (
          <p className="muted">Er zijn op dit moment geen tijden beschikbaar. Neem gerust contact op.</p>
        )}
        {slots &&
          slots.map((slot) => {
            ...
            return (
              <div className="card" key={slot.id}>
                <div className="card-row">
                  ...
                  <button
                    className="btn"
                    onClick={() => setOpenSlotForm(openSlotForm === slot.id ? null : slot.id)}
                  >
                    Plan in
                  </button>
                </div>
                {openSlotForm === slot.id && (
                  <div style={{ marginTop: "1.25rem", borderTop: "1px solid var(--border)", paddingTop: "1.25rem" }}>
                    <BookingForm
                      submitLabel="Bevestig afspraak"
                      submitting={state.submitting}
                      error={state.error}
                      success={state.success}
                      onSubmit={(form) => submitSlotBooking(slot, form)}
                    />
                  </div>
                )}
              </div>
